    ...
    std::thread([this]() {
        ...
        while (true) {
            ...
            xiaomiFingerprintService->extCmd(COMMAND_NIT,
                    readBool(fd) && mFodCircleVisible ? PARAM_NIT_FOD
                    : PARAM_NIT_NONE);
        }
    }).detach();
}
...
Return<bool> FingerprintInscreen::handleAcquired(int32_t acquiredInfo, int32_t vendorCode) {
    ALOGD("handleAcquired() acquiredInfo: %d vendorCode: %d", acquiredInfo, vendorCode);
    return false;
}

Return<bool> FingerprintInscreen::handleError(int32_t error, int32_t vendorCode) {
    ALOGE("handleError() error: %d vendorCode: %d", error, vendorCode);
    return false;
}
